...
**Warum der Volltext per `Read` geladen wird und nicht über `/session-protocol`.** Derselbe Key hat eine zweite Folge, die bis 0.8.0 unbemerkt blieb: Ein Skill mit `disable-model-invocation: true` erscheint im Skill-Listing einer Session gar nicht, ist also für das Modell nicht aufrufbar (Doku: „prevent Claude from automatically loading this skill. Use for workflows you want to trigger manually with `/name`"). Die Kurzfassung verwies aber genau dorthin — zwei parallele Sessions haben den Volltext deshalb nie geladen und konnten anschließend nicht sagen, ob sie Regeln verletzt hatten. Seit 0.9.0 nennt sie den Dateipfad als Ladeweg und `/session-protocol` nur noch als Weg für den Nutzer. Der Auslöser ist der **erste Kontakt**, nicht die erste ausgehende Nachricht: Eingehende Nachrichten kommen unangekündigt, und die Empfangsregeln sind gerade die, die ohne Volltext fehlen.

**Warum die Kurzfassung trotzdem schlank bleibt.** Sie kostet in *jeder* Session dieses Projekts Kontext, der Volltext nur in kommunizierenden. Die drei neuen Regeln (Nachreichpflicht, keine Arbeitszuweisung, kein Warten) sind deshalb als Halbsätze an bestehende Punkte gehängt statt als eigene Punkte, und der Ladeauslöser sitzt in der Kopfzeile, die ohnehin korrigiert werden musste. Der Volltext trägt ein Zeilenbudget von rund 120 Zeilen; die Ergänzungen wurden teilweise durch Straffen von „Warum es das gibt" gegenfinanziert, das ans Ende gerückt ist, weil operative Regeln vorn stehen sollen.

**Warum das Zustellverhalten Protokolltext ist und kein Mechanismus.** Belegt ist nur die Semantik „messages enqueue and drain at the receiver's next tool round"; dass eine Session im Freigabedialog keinen Tool-Zug macht, ist eine Ableitung aus einer Beobachtung und im Text als solche gekennzeichnet. Die Blockade liegt damit nicht im Transport, sondern im Wartezustand des Empfängers — ein Retry verschlimmert sie nur, und ein Mechanismus (Quittungen, Statusdatei) fügte genau das Ping-Pong hinzu, das das Protokoll verbietet. Was hilft, ist eine Regel über den Sendezeitpunkt. Die zweite mögliche Ursache steht daneben, weil sie in fremden Projekten die wahrscheinlichere ist: Ohne `crossSessionInbound: "accept"` wird gehalten statt zugestellt.

...
- **Fahr die `project-*`-Kette nicht in einer Session.** Installiert kosten die Skills nichts: Ein Sessionstart mit dem Plugin und einer ohne unterscheiden sich um 0 Tokens. Aufgerufen bleibt dagegen der ganze Skilltext bis zum Sessionende im Fenster, und die vier `project-*`-Skills summieren sich auf rund 33.000 Tokens, mit ihren nachgeladenen Katalogen auf bis zu 59.000. Nimm einen Schritt je Session, dann kostet dich nur der, den du gerade brauchst. Die Messung dahinter steht in [`DESIGN.md`](../DESIGN.md) unter „Kontextlast der Skills".

...
- **`project-structure` schreibt genau einen Abschnitt in die `CLAUDE.md`**, den Wegweiser unter der festen Überschrift `## Ablage`. Den Rest der Datei fasst es nicht an. Fehlt die Datei und auch eine `AGENTS.md`, legt es die `CLAUDE.md` mit nichts als diesem Abschnitt an und nennt den Rückweg `rm`.
...
- **`session-handoff` schreibt genau eine Datei**, bei einer untrackten Übergabedatei vorher deren `.bak`-Kopie, und hört dort auf — er setzt die Arbeit nicht fort, auch nicht auf Bitte. Die Datei ist die Übergabe; im neuen Fenster nimmt `/cmd:session-resume` sie auf. Das Fenster öffnest du selbst: Der Skill tut nach der Datei nichts mehr.

...
**Danach:** Empfiehl `/cmd:project-rules`, wenn der Wegweiser neu ist oder Inhalt aus der `CLAUDE.md` abgeflossen ist — die Datei ist dann inhaltlich richtig, aber weder gehärtet noch verdichtet. Sag dabei ausdrücklich, wo dieser Lauf in der Reihe steht: `project-settings` → `project-structure` → `project-audit` → `project-rules` → `project-curate`, und du bist beim zweiten Glied. Der **letzte** ist `project-curate`: Findet `project-rules` danach noch auslagerungsreife Blöcke, sind es solche, die dieser Lauf bewusst hat liegen lassen — dann ist die Antwort eine Entscheidung darüber, nicht ein weiterer Lauf von `project-structure`. Schick den Nutzer nicht im Kreis.

---
name: session-learn
description: Wertet den Verlauf der laufenden Session aus und leitet daraus dauerhafte, belegbare Learnings für künftige Sessions ab, routet jedes an den passenden projektlokalen Ort (Projekt-CLAUDE.md, references oder Repo, nie user-global; nutzt das Memory-System nicht) und fasst sie in einen Plan, den plan-review härtet und plan-execute anwendet. Schreibt selbst nichts an die Zielorte. Gedacht als End-of-Session-Retro, eigenständig nutzbar.
argument-hint: "[optionaler Fokus]"
disable-model-invocation: true
---

Rolle und Grenze. Du wertest den Verlauf der laufenden Session aus und leitest daraus Learnings ab, die künftige Sessions besser machen. Gearbeitet wird an dem, was im Verlauf belegt steht — an Aufträgen, Korrekturen, Tool-Ergebnissen und Dateizuständen —, nicht an einer Nachschau auf dein eigenes Denken. Du schreibst dabei nichts an die Zielorte — kein Memory, keine CLAUDE.md, keinen Skill, keine Projektdatei. Deine einzige Ausgabe ist ein Plan: fasse die Learnings im Plan-Modus in die Plandatei und verweise den Nutzer auf plan-review und plan-execute; beide rufst nicht du auf, sondern er. Du bist damit ein /plan, das die Session als Eingabe nimmt, nicht der, der die Learnings schon einarbeitet. Gedacht als End-of-Session-Retrospektive: schließe die laufende Aufgabe erst ab und committe sie, denn dein Plan belegt die Plandatei und ersetzt den bisherigen Plan-Kontext.
...
Woran ein Learning sichtbar wird. Das stärkste Signal sind die Stellen, an denen der Nutzer dich korrigiert, unterbrochen oder umgelenkt hat — dort steckt das wertvollste Learning, und es ist im Verlauf nachlesbar. Richte den Blick auf das, was Claude künftig anders machen soll, nicht auf das, was der Nutzer anders machen müsste. Benenne einen belegten Fehlgriff so, wie er im Verlauf steht, statt ihn kleinzureden oder zu einer glatten Bilanz zu runden.
...
Übergabe. Verweise den Nutzer auf `/cmd:plan-review` zum Härten und danach `/cmd:plan-execute` zum Anwenden; beide kann nur er aufrufen, ein Aufruf durch dich ist gesperrt. Beende den Zug. Die Ziele liegen projektlokal im Projekt; ist es versioniert, ist der Rückweg `git restore` — zusätzlich erlaubt die stabile Kennung die gezielte logische Rücknahme.

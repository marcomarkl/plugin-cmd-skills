...
Die letzte Zeile ist der Vollständigkeitsbeleg: Sind alle Katalogregeln in einer der Kategorien gelandet, ist die Datei vollständig und gehärtet — ohne dass eine starke Regel geschwächt wurde. Den Nachweis trägt das Register aus Schritt 4, nicht diese Zeile; sie referiert es nur. **Tokeneffizient ist sie damit noch nicht:** Das Verdichten ist ein zweiter Aufruf, und der heißt `/cmd:project-curate`. Nenne ihn am Ende deines Protokolls, sonst erfährt niemand von ihm, der diesen Skill direkt aufgerufen hat.

...
## 10. Prüfen, was von außen prüfbar ist
...
- `validate` prüft nur die Manifeste, nicht ob ein Skill wirkt. Bei geändertem Skill-Verhalten zusätzlich `claude --plugin-dir ./cmd`, `/reload-plugins`, Skill aufrufen (siehe 5.). Betrifft die Änderung eine **Verzweigung**, stell den auslösenden Zustand in einem Wegwerf-Verzeichnis her und ruf den Skill dort headless auf (`claude --plugin-dir <abs. Pfad> -p "/cmd:<skill>"`): Das belegt beide Äste einzeln, kostet einen Bruchteil von `scripts/smoke.sh` und bleibt ohne Nebenwirkung, weil headless die Freigabe fehlt, an der die schreibenden Skills hängen.
- **Zahlwörter im Fließtext gehören zur Änderung.** Hier wird an vielen Stellen mitgezählt — „zwölf Skills", „zehn Kataloge", „vier Spuren", die Klammerzahlen der Kopien-Checkliste in `DESIGN.md`. Dass diese Beispiele selbst zweimal veralteten, ist der beste Beleg für die Regel. Ändert sich eine Aufzählung, zieh die Zahl davor mit und zähl gegen die tatsächliche Länge nach: `validate` fängt das nicht, und eine falsche Summe entwertet die Liste, die sie beschreibt.
...
- **Was hier bewusst fehlt:** ein Schritt, der den eigenen Entwurf noch einmal gegen den Auftrag liest. Der Opus-5-Leitfaden empfiehlt, solche Anweisungen zu entfernen — sie erzeugen Über-Verifikation, ohne die Qualität zu heben —, und der Wortlaut stand bis 0.24.0 auch im Plugin (`ausfuehrungsdisziplin.md`), wo ihn Etappe 2 des Leitfaden-Abgleichs gestrichen hat. Inhaltlich trägt ihn Abschnitt 7: Annahmen benennen, Unbelegtes als unsicher kennzeichnen. Die Prüfungen oben bleiben, weil ihr Kriterium von außen kommt.

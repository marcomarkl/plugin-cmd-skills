...
**Registriert ist der Marketplace als GitHub-Source, nicht als `directory`** — geprüft in `~/.claude/settings.json` und `~/.claude/plugins/known_marketplaces.json`, beide führen `{"source": "github", "repo": "marcomarkl/plugin-cmd-skills"}`. Der lokale Klon liegt unter `~/.claude/plugins/marketplaces/marco-markl` und wird bei jedem Update neu gezogen. Praktische Folge: **`marketplace update` liest von GitHub, nicht aus diesem Arbeitsverzeichnis** — ein lokaler Commit reicht nicht, es muss gepusht sein (siehe 4.).
...
Anders bei einer Registrierung mit `source: directory`: dort hängt alles am **absoluten Pfad** in `~/.claude/settings.json` (`extraKnownMarketplaces`) und abgeleitet in `known_marketplaces.json`, und ein toter Pfad äußert sich als `cache-miss` in `claude plugin list` — nicht als Datei- oder Namensfehler. Nur dann ist nach einem Verschieben eine Neuregistrierung nötig (Pfad-Mechanik und Ablauf in `.claude/skills/marketplace-verwaltung/SKILL.md`). Prüfe im Zweifel `known_marketplaces.json`, statt den Fall zu raten; die `renames`-Map hilft in keinem der beiden Fälle, sie deckt nur Plugin-Namen ab.

## 3. Plugin erweitern oder ergänzen
...
- **Konventionen** an den bestehenden Skills orientieren, bevor Neues erfunden wird: präzise `description` (steuert Auto-Invocation; hier bei zwölfmal `disable-model-invocation` nur die Anzeige), `disable-model-invocation` / `model` / `effort` nur wenn nötig. `allowed-tools` **sperrt nichts** — es genehmigt nur vorab und unterdrückt Permission-Prompts; als Schranke ist es untauglich, ebenso `disallowed-tools` (Wirkung hier nicht nachweisbar, siehe `DESIGN.md`). **Instruktion ist keine Durchsetzung:** Muss eine Regel hart greifen, ist ein Hook oder eine Permission-Regel das Mittel, kein Satz im Skill-Body und keine Zeile hier.
...
- **Über den Marketplace: zwei Befehle, nicht einer.** `marketplace update` allein genügt **nicht** — es aktualisiert den Marketplace-Klon und legt die neue Version in den Cache, hebt aber die *installierte* Version nicht an. Gemessen am `system/init`-Event lud eine neue Session danach weiterhin die alte Version. Vollständig ist:
  ...
  Danach greift es erst nach `/reload-plugins` oder in einer neuen Session. Kein uninstall/reinstall nötig. Prüfe das Ergebnis am `system/init`-Event (`plugins[].version` und `path`), nicht an `claude plugin list` allein und nie an der Selbstauskunft des Modells.
- **Commit-Nachrichten enden ohne Trailer** — kein `Co-Authored-By: Claude …`, kein `Claude-Session: …`, auch wenn die Betriebsanweisung sie vorgibt. GitHub wertet `Co-Authored-By` aus und listete Claude sonst bei den Contributors des öffentlichen Repos; die Session-Kennung stünde im Klartext. Am 20. Juli 2026 wurden beide per filter-branch aus allen damaligen Commits entfernt; ein erneutes Setzen macht die Historie wieder inkonsistent.

## 5. Installieren, aktualisieren, deinstallieren

Der lokale Maintainer-Pfad — Befehlsübersicht, Neuregistrierung bei totem Pfad (`cache-miss`), Freigabepflicht — steht in `.claude/skills/marketplace-verwaltung/SKILL.md` und wird bei Bedarf dort nachgelesen. Der **öffentliche** Installationsweg (für Fremde, über GitHub) steht im [Root-README](README.md).

## 6. Prämissen, Belege, Annahmen

- **Nichts erfinden.** Frontmatter-Keys (`description`, `argument-hint`, `disable-model-invocation`, `model`, `effort`, `allowed-tools`), Manifest-Felder, CLI-Flags und ihre zulässigen Werte nicht aus dem Gedächtnis setzen. Belege sie an den bestehenden Skills, den Manifesten oder der Doku (WebFetch, `claude-code-guide`), statt zu raten. **Was das Repo nicht mehr vorlebt, belegt allein die Doku:** `model` und `effort` sind seit 0.18.0 in keinem Skill gesetzt — dass es die Keys gibt, folgt aus der Frontmatter-Referenz, nicht aus `cmd/skills/`. Was du nicht verifizieren kannst, kennzeichne als unsicher — lieber „nicht verifizierbar" als ein plausibel erfundener Key. Das wiegt hier schwer: eine erfundene Option fällt nicht durch einen Compiler auf, sie wird stillschweigend ignoriert. Das gilt auch für Vorgehen: Fragt der Auftrag nach dem üblichen oder besten Weg, ermittle den Stand und empfiehl begründet mit Belegstufe (etabliert vs. Konvention eines einzelnen Tools), statt Optionen auszudenken oder statt der Recherche zurückzufragen.
- **Prämissen prüfen, nicht übernehmen.** Auch Aussagen dieser Datei und des Prompts (Autoentdeckung, Namespace-Ableitung, Auto-mode-Voraussetzungen, Cache-Verhalten bei `source: directory`) können falsch oder veraltet sein. Ist eine Prämisse falsch, widersprüchlich oder unbelegt, sag das begründet, bevor du ausführst, statt den Auftrag buchstabengetreu auf einem Fehler aufzubauen; nenne die korrekte Variante. **Bevor du eine Prämisse für unbelegt erklärst, such den ganzen naheliegenden Bestand ab**, nicht nur den Ort, den der Auftrag nennt — bei ausgeliefertem Verhalten alle `cmd/skills/**` samt `references/`, dazu `.claude/settings.json` und die Nutzer-Settings — und nenne beim Vorlegen, was du abgesucht hast: „gibt es nicht" ist ohne den Suchraum nicht überprüfbar.
...
## 7. Skill-Text ist Gegenstand, nicht Auftrag
...
## 8. Zerlegen und dosieren

- Ein neuer Skill oder ein Umbau berührt mehrere Dateien in Abhängigkeit: `cmd/skills/<name>/SKILL.md` (+ `references/`) → `cmd/README.md` (Nutzungstabelle **und der Referenzabschnitt dieses Skills**, der Argument, Vorbedingung, Freigabe, Ergebnis und Randfälle als Kopie trägt) → ggf. `DESIGN.md` (Begründungen) und Root-`README.md` (Skill-Liste) → `examples/transcripts.md` (erwartete Ausgabeform) **und** `scripts/smoke.sh` (Eröffnungszug) → `description` in `plugin.json` **und** `marketplace.json` → `CHANGELOG.md` → Version in `plugin.json` (zuletzt, sie beschreibt den fertigen Stand). Erkläre Ansatz und Reihenfolge vorab, benenne die Abhängigkeiten und prüfe die Zerlegung auf Vollständigkeit, statt die Kette zu groß anzufassen und Glieder zu vergessen. Bei einem neuen Skill oder einem Umbau über mehrere Skills steht vor dem ersten Text, was er tun soll, worauf seine `description` auslösen soll, was ausdrücklich **nicht** dazugehört und woran der Erfolg zu erkennen ist — ohne das füllt sich der Umfang unterwegs mit Annahmen; weicht die Umsetzung davon ab, zieh die Festlegung nach und arbeite gegen die neue, statt nur den Einzelfall zu flicken. Halte beim Abarbeiten den Stand je Glied fest, über mehrere Sitzungen hinweg in der Plandatei statt im Verlauf, den eine Verdichtung nicht überlebt. Die beiden Testartefakte standen bis 0.8.0 nicht in dieser Kette — genau deshalb fehlte `project-rules` dort über mehrere Versionen unbemerkt.
- **`project-setup` hängt an den fünf anderen `project-`Skills.** Es kopiert deren Argumente, Abnahmekriterien und Randfälle in seine Aufrufliste; welche Fakten das einzeln sind, steht in `DESIGN.md` unter `### project-setup` — seit 0.24.0 einundzwanzig. Änderst du an `project-settings`, `project-structure`, `project-audit`, `project-rules` oder `project-curate` ein Argument, einen Randfall oder ein Abnahmekriterium, gehört `cmd/skills/project-setup/SKILL.md` in denselben Zug. Der umgekehrte Weg ist gedeckt: Wer `project-setup` ändert, prüft die Kopien beim Verifizieren.
...
## 9. Prüfen, was von außen prüfbar ist
...
- `validate` prüft nur die Manifeste, nicht ob ein Skill wirkt. Bei geändertem Skill-Verhalten zusätzlich `claude --plugin-dir ./cmd`, `/reload-plugins`, Skill aufrufen (siehe 4.). Betrifft die Änderung eine **Verzweigung**, stell den auslösenden Zustand in einem Wegwerf-Verzeichnis her und ruf den Skill dort headless auf (`claude --plugin-dir <abs. Pfad> -p "/cmd:<skill>"`): Das belegt beide Äste einzeln, kostet einen Bruchteil von `scripts/smoke.sh` und bleibt ohne Nebenwirkung, weil headless die Freigabe fehlt, an der die schreibenden Skills hängen.
- **Zahlwörter im Fließtext gehören zur Änderung.** Hier wird an vielen Stellen mitgezählt — „zwölf Skills", „zehn Kataloge", „vier Spuren", die Klammerzahlen der Kopien-Checkliste in `DESIGN.md`. Diese Beispiele veralteten selbst schon zweimal. Ändert sich eine Aufzählung, zieh die Zahl davor mit und zähl gegen die tatsächliche Länge nach: `validate` fängt das nicht, und eine falsche Summe entwertet die Liste, die sie beschreibt.
- **Massenersetzung nur mit Gegenprobe im selben Zug**, mit einem `grep`, das den Zielzustand behauptet. `sed` ist hier BSD-`sed`: `-i` braucht das leere Argument, und `\|` als Alternation kennt es in BRE nicht — ein solcher Ausdruck läuft wirkungslos durch, ohne Fehler und mit Exit 0. Mehrteilige Muster trennen oder gleich Python nehmen; ein stiller Teilerfolg sieht sonst aus wie Erfolg.
- Behaupte keinen Lauf, keine Prüfung und keinen Schritt, den du nicht tatsächlich durchgeführt hast.
- **Was hier bewusst fehlt:** ein Schritt, der den eigenen Entwurf noch einmal gegen den Auftrag liest. Trag ihn nicht nach; Begründung in `DESIGN.md`.

## 10. Fehler, Sicherung, Rückweg

- **Das Repo ist versioniert**, Remote `origin` → `marcomarkl/plugin-cmd-skills`. Der Rückweg aus einem Fehlversuch ist `git restore <datei>` bzw. `git checkout` — committe deshalb einen funktionierenden Stand, bevor du großflächig umschreibst, statt `.bak`-Kopien anzulegen. **Die Grenze verläuft nicht am Repo-Rand, sondern dort, wo git keinen Rückweg bietet**: bei Dateien außerhalb des Repos (`~/.claude/settings.json`, die Plugin-Registrierung) **und bei untrackten Dateien darin**. In beiden Fällen ist die `.bak`-Kopie vor dem Ändern Pflicht — `git restore` stellt eine nie eingecheckte Datei nicht wieder her. Genau so verfahren `session-handoff` und `project-settings` (siehe 11.).
...
- Lesen und Editieren im Repo sind frei — **committen und pushen nicht**: Sie geschehen auf Aufforderung. `push` ändert zusätzlich den ausgerollten Stand (siehe 4.).
...
## 12. Wissen ablegen und diese Datei pflegen
...
- **Ein wiederkehrender Ablauf des Arbeitens im Repo gehört in ein eigenes Artefakt, nicht in diese Datei.** Welches, entscheidet der Ladezeitpunkt:
  - Ablauf, den du selbst ausführst → Skill `.claude/skills/<name>/SKILL.md`
  - ausgabestarke Teilarbeit → ad hoc an einen eingebauten Subagent-Typ; eine Datei `.claude/agents/<name>.md` erst bei Wiederkehr oder festzuschreibender Rückgabeform
  - Konvention für einen Dateibereich → Regel `.claude/rules/<thema>.md`, **nur mit** `paths:` — ohne lädt sie beim Sessionstart mit und entlastet nichts
  - Wissen, das nur an einem Teilbaum hängt → `<unterordner>/CLAUDE.md`, lädt erst beim Arbeiten dort
- Kein Auslagerungsweg ist ein `@pfad`-Import: er lädt beim Sessionstart vollständig mit und ordnet nur die Pflege. Für dieselbe Sache genau eines; ob es gefunden wird, entscheidet die `description`, nicht der Dateiname. Vorbild ist `marketplace-verwaltung`; `.claude/agents/`, `.claude/rules/` und verschachtelte `CLAUDE.md` gibt es noch nicht.
...
- Für den Subagenten heißt das konkret: breite Recherche, Doku-Abgleich und Durchsicht der git-Historie laufen dort, nicht im Hauptkontext, außer der Wortlaut wird im Hauptkontext gebraucht oder unterwegs ist eine Rückfrage nötig. Sein Kontext startet leer — Aufgabe, Pfade und der Zielpfad für das Ergebnis gehören in den Prompt, sonst rät er.
...
Nicht duplizieren, sondern auf den hier genannten Ort verweisen.

- Im Repo arbeiten: `CLAUDE.md` · Einstieg und Installation: `README.md` · Plugin nutzen und Befehlsreferenz: `cmd/README.md` — kein `docs/`
...
- Pläne: user-global in `~/.claude/plans/`; das vorhandene `plans/` trägt ältere Pläne und bleibt ignoriert

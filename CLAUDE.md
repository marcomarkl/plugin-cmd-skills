...
- **`project-setup` hängt an den drei anderen `project-`Skills.** Es kopiert deren Argumente, Abnahmekriterien und Randfälle in seine Aufrufliste; welche Fakten das einzeln sind, steht in `DESIGN.md` unter `### project-setup`. Änderst du an `project-settings`, `project-structure` oder `project-rules` ein Argument, einen Randfall oder ein Abnahmekriterium, gehört `cmd/skills/project-setup/SKILL.md` in denselben Zug. Der umgekehrte Weg ist gedeckt: Wer `project-setup` ändert, prüft die Kopien beim Verifizieren.
...
- **Zahlwörter im Fließtext gehören zur Änderung.** Hier wird an vielen Stellen mitgezählt — „zehn Skills", „sieben Kataloge", „vier Spuren", die Klammerzahlen der Kopien-Checkliste in `DESIGN.md`. Ändert sich eine Aufzählung, zieh die Zahl davor mit und zähl gegen die tatsächliche Länge nach: `validate` fängt das nicht, und eine falsche Summe entwertet die Liste, die sie beschreibt.
